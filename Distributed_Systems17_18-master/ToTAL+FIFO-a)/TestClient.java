            ...
            try {
                ...
                while (true) {
                    ...
                    if (sentence.startsWith("!addme")) {
                        ...
                        InetSocketAddress address1 = new InetSocketAddress(UDPAddress, Integer.parseInt(PORT)+2);
                        ...
                        if( !temp.getClients().isEmpty() && temp.getClients().size()==seqnumberspermessage.get(messageID).size()){
                            ...
                            for(Iterator<Group> it=mygroups.iterator(); it.hasNext();) {
                                ...
                                for (TestClient i : j.getClients()) {
                                    DatagramSocket mySocket = new DatagramSocket(null);
                                    InetSocketAddress address1 = new InetSocketAddress(UDPAddress, (Integer.parseInt(PORT) + 3));
                                    mySocket.bind(address1);
...
/////////
    ...
    private void run() throws IOException {
        ...
        BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\HP-User\\IdeaProjects\\First\\src\\messages5.txt"));
                                ...
                                if (number > 1) {
                                    for (TestClient j : group.getClients()) {
                                        if (j.getID() != id) {
                                            DatagramSocket mySocket = new DatagramSocket(null);
                                            //DatagramSocket serverSocket = new DatagramSocket(null);
                                            InetSocketAddress address = new InetSocketAddress(this.getUDP(), Integer.parseInt(this.getPORT())+4);
                ...
                if( !command.isEmpty()) {
                    if (mygroups != null) {
                        if (writegroup != null) {
                            ...
                            while(writegroup.getClients().size()<5){
                                try{
                                    Thread.sleep(500);
                                }catch(Exception e){
