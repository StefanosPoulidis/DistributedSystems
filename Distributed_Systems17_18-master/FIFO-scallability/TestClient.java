...
public class TestClient {
    ...
    private static ArrayList<String> holdBackQueue = new ArrayList<String>();
            ...
            try {
                ...
                while (true) {
                    ...
                    if (sentence.startsWith("!addme")) {
                        ...
                        String[] split = sentence.split("!@#");
                        //!message!@#username!@#groupname!@#messageId!@#in GROUP user USER says: TEXT ( pou vazw to messageId gia na exw fifo sigoura )
                        String user = split[1];
                        String groupname = split[2];
                        int messageid = Integer.parseInt(split[3]);
                        String text = split[4];
                        //elegxoume gia fifo edw
                        int temp = vectorpergroup.get(groupname).get(user);
                        if (temp + 1 == messageid) {
                            final long startTime1 = System.currentTimeMillis();
                            System.out.print("RECEIVED at "+startTime1+" :");
                            System.out.println(text);
                            vectorpergroup.get(groupname).put(user, messageid);
                        } else if (temp + 1 < messageid) {
                        ...
                        if (!holdBackQueue.isEmpty()) {
                            for (Iterator<String> it = holdBackQueue.iterator(); it.hasNext(); ) {
                                String i = it.next();
                                String[] splitqueuestring = i.split(",");
                                String userofmessage = splitqueuestring[1];
                                String groupofmessage = splitqueuestring[2];
                                int idofmessage = Integer.parseInt(splitqueuestring[3]);
                                String textofmessage = splitqueuestring[4];
                                for (Iterator<Group> it1 = mygroups.iterator(); it1.hasNext(); ) {
                                    Group j = it1.next();
                                    String namej = j.getGroupname();
                                    if (namej.equals(groupofmessage)) {
                                        for (Iterator<TestClient> it2 = j.getClients().iterator(); it2.hasNext(); ) {
                                            TestClient k = it2.next();
                                            if (k.getUsername().equals(userofmessage)) {
                                                if (vectorpergroup.get(groupofmessage).get(userofmessage) + 1 == idofmessage) {
        ...
        while (true) {
            ...
            if (command.startsWith("!")) {
                ...
                if (command.equals("!help")) {
                    System.out.println("Available commands: !help, !register, !lm, !lg, !w, !e, !q");
                        ...
                        if (writegroup != null) {
                            ...
                            if (command.equals("OK")) {
                                ...
                                while (command != null) {
                                    ...
                                    String premessage = "!message!@#" + this.getUsername() + "!@#" + writegroup.getGroupname() + "!@#" + messageIdpergroup.get(writegroup.getGroupname()) + "!@#";
                                    String sendmessage = premessage + message;
                                    for (TestClient i : writegroup.getClients()) {
                                        DatagramSocket mySocket = new DatagramSocket(null);
                                        InetSocketAddress address = new InetSocketAddress(this.getUDP(), 60000);
                                        mySocket.bind(address);
                                        DatagramPacket datagramPacket = new DatagramPacket(
